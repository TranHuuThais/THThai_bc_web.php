...
function createUsers()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($errors)) {
            ...
            $queryInsert = "INSERT INTO user (fastname, name, email, password, address, phone) 
...
function getProduct($id)
{
    ...
    if (!$stmt) {
        ...
        return "Prepared statement creation failed: " . mysqli_error($conn);
    }
    ...
    if (!$success) {
        ...
        return "Statement execution failed: " . mysqli_error($conn);
    }
...
{
    global $conn;
    $query = "SELECT * FROM products WHERE VIEW > 50 ORDER BY VIEW DESC LIMIT 50";
    // 3. Thực thi câu truy vấn SQL để lấy về dữ liệu
    $result = mysqli_query($conn, $query);
    // 4. Khi thực thi các truy vấn dạng SELECT, dữ liệu lấy về cần phải phân tách để sử dụng
    // Thông thường, chúng ta sẽ sử dụng vòng lặp while để duyệt danh sách các dòng dữ liệu được SELECT
...
function createProduct()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        echo "$name  $categoryID  $price $view $description $image";
        ...
        if (empty($errors)) {
            ...
            $queryInsert = "INSERT INTO products (name, categoryID, price, description, image, view, created_at) 
            VALUES ('$name', '$categoryID', '$price', '$description', '$image', '$view', '$created_at')";
...
function updateProducts($conn, $id, $name, $categoryID, $price, $description, $image, $view, $created_at)
{
    $name = mysqli_real_escape_string($conn, $name);
    ...
    $queryUpdate = "UPDATE products SET name = '$name', categoryID = '$categoryID', price ='$price',  description = '$description', image = '$image',view='$view',created_at = '$created_at'  WHERE id='$id'";
...
function createCategories()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        $errors =  validationCreateProduc($name, $image ,$description,);
        if (empty($errors)) {

            $queryInsert = "INSERT INTO products (name, categoryID, price, description, image, view, created_at) 
            VALUES ('$name', '$categoryID', '$description,'$price',  '$image', '$view', '$created_at')";
...
function updateCategories($conn, $id, $name,$image, $description,$price, $updated_at)
{
    $name = mysqli_real_escape_string($conn, $name);
    $description = mysqli_real_escape_string($conn, $image);
    $description = mysqli_real_escape_string($conn, $description);
    $description = mysqli_real_escape_string($conn, $price);
    $updated_at = mysqli_real_escape_string($conn, $updated_at);


    $queryUpdate = "UPDATE categories SET name = '$name',image = '$image' description = '$description',price = '$price', updated_at = '$updated_at' WHERE id='$id'";
...
function createOrders()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        $queryInsert = "INSERT INTO orders (total, phone, address) VALUES ('$phone','$address','$total')";
